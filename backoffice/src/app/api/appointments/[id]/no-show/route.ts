...
export async function PUT(
    ...
    try {
        ...
        const response = await serviceRequest({
            method: 'PUT',
            path: `/appointments/admins/${id}/no-show`,
            token: h.token,
        });
        return NextResponse.json(response, { status: response.status > 0 ? 200 : 400 });
    } catch (error) {
        console.error('appointment no-show', { error });
        return NextResponse.json({ status: 0, message: 'Error al marcar ausencia' }, { status: 500 });
    }
}
