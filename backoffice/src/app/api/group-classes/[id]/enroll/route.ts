...
export async function POST(
    ...
    try {
        ...
        const response = await serviceRequest({
            method: 'POST',
            path: `/group-classes/${id}/enroll`,
            data,
            token: h.token,
        });
        return NextResponse.json(response, { status: response.status > 0 ? 201 : 400 });
    } catch (error) {
        console.error('group-classes enroll POST', { error });
        return NextResponse.json({ status: 0, message: 'Error al inscribir cliente en clase' }, { status: 500 });
    }
}
