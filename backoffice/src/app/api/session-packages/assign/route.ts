...
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await serviceRequest({
            method: 'POST',
            path: '/session-packages/assign',
            data,
            token: h.token,
        });
        return NextResponse.json(response, { status: response.status > 0 ? 201 : 400 });
    } catch (error) {
        console.error('session-packages assign POST', { error });
        return NextResponse.json({ status: 0, message: 'Error al asignar paquete' }, { status: 500 });
    }
}
