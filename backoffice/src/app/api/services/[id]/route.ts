...
export async function GET(
    ...
    try {
        ...
        const response = await serviceRequest({
            method: 'GET',
            path: `/services/admins/${id}`,
            token: h.token,
        });
        return NextResponse.json(response, { status: response.status > 0 ? 200 : 400 });
    } catch (error) {
        console.error('service GET by id', { error });
        return NextResponse.json({ status: 0, message: 'Error al obtener servicio' }, { status: 500 });
    }
}
...
export async function PUT(
    ...
    try {
        ...
        return NextResponse.json({ status: 0, message: 'Error al actualizar servicio' }, { status: 500 });
    }
}
...
export async function DELETE(
    ...
    try {
        ...
        return NextResponse.json({ status: 0, message: 'Error al eliminar servicio' }, { status: 500 });
    }
}
