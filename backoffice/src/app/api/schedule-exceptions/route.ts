...
export async function GET(request: NextRequest) {
    try {
        ...
        const response = await serviceRequest({
            method: 'GET',
            path: '/schedule-exceptions/admins',
            params,
            token: h.token,
        });
        ...
        return NextResponse.json({ status: 0, message: 'Error al obtener excepciones' }, { status: 500 });
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        return NextResponse.json({ status: 0, message: 'Error al crear excepcion' }, { status: 500 });
    }
}
