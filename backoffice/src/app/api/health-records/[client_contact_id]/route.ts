...
export async function GET(request: NextRequest, { params }: { params: Promise<{ client_contact_id: string }> }) {
    ...
    try {
        ...
        const response = await serviceRequest({
            method: 'GET',
            path: `/health-records/${client_contact_id}`,
            token: h.token,
        });
        return NextResponse.json(response, { status: response.status > 0 ? 200 : 400 });
    } catch (error) {
        console.error('health-records GET', { error });
        return NextResponse.json({ status: 0, message: 'Error' }, { status: 500 });
    }
}

export async function PUT(request: NextRequest, { params }: { params: Promise<{ client_contact_id: string }> }) {
    const { client_contact_id } = await params;
    try {
        const h = getRequestHeaders(request);
        if (!h.token) return NextResponse.json(not_auth, { status: 401 });
        ...
        console.error('health-records PUT', { error });
        return NextResponse.json({ status: 0, message: 'Error' }, { status: 500 });
    }
}
