...
/**
 * Resuelve las credenciales de Mercado Pago desde variables de entorno:
 * MP_ACCESS_TOKEN (requerida), MP_PUBLIC_KEY y MP_WEBHOOK_SECRET (opcionales).
 ...
 *
 * @param {Object} opts
 * @param {Object} opts.appointment - turno ya persistido
 * @param {Object} opts.service - servicio con precio
...
export async function testMercadoPagoCredentials() {
    const creds = resolveMPCredentials();
    if (!creds) return { ok: false, error: 'MP no esta configurado (MP_ACCESS_TOKEN)' };
    try {
