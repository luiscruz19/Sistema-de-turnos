...
/**
 * Resuelve la API key de OpenAI desde variables de entorno.
 * OPENAI_API_KEY (requerida) y OPENAI_MODEL (opcional).
 ...
 * Las credenciales se leen de variables de entorno (OPENAI_API_KEY / OPENAI_MODEL).
 *
...
export async function testOpenAICredentials() {
    const creds = resolveOpenAICredentials();
    if (!creds) {
        return { ok: false, error: 'OpenAI no esta configurado (OPENAI_API_KEY)' };
    }
    try {
